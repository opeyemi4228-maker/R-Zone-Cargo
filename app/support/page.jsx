...
import {
  ...
  AlertCircle, Loader2, Send, X, Star,
  LifeBuoy, BookOpen, Zap, HelpCircle,
  Globe, Instagram, ExternalLink, RefreshCw,
  TrendingUp, Hash,
} from "lucide-react";
...
const FAQ_CATEGORIES = [
  {
    ...
    color: "#0818A8",
    faqs: [
      { q: "How do I book a shipment with R-Zone?", a: "You can book online via our Get a Quote form, call us on +44 800 772 0864, email info@r-zoneenterprises.com, or visit our Essex warehouse in person. We confirm all bookings in writing with a unique reference number." },
      ...
      { q: "How far in advance do I need to book sea freight?", a: "We recommend booking at least 5 working days before the published cut-off date to guarantee space on the sailing. The cut-off is typically 5 days before the vessel departure date." },
  ...
  },
  {
    ...
    color: "#1F51FF",
    faqs: [
      { q: "How do I track my shipment?", a: "Use our Track Shipment tool at r-zoneenterprises.com/track with your booking reference number (format: RZC-XXXX-XXXXX). You can also call +44 800 772 0864 or WhatsApp us for a live update." },
  ...
  },
  {
    ...
    color: "#0818A8",
    faqs: [
      { q: "Do you collect from my home in the UK?", a: "Yes — we offer door collection across the entire UK for an additional fee based on your location. Our Essex warehouse also accepts drop-offs Mon–Fri 9am–6pm and Sat 10am–2pm." },
      ...
      { q: "Can I collect my cargo from your Lagos warehouse?", a: "Yes — recipients can collect from our Igando, Lagos facility. Bring a valid ID and your booking reference. Call our Lagos team on arrival to arrange assisted collection." },
...
const CONTACT_CHANNELS = [
  ...
  },
  {
    ...
    href: "https://instagram.com/rzoneenterprise",
...
const STATUS_ITEMS = [
  ...
  { label: "Sea Freight Sailings",   status: "operational", detail: "Next sailing: 8 Oct — spaces available"  },
...
const fadeUp = {
  ...
  visible: (d = 0) => ({ opacity: 1, y: 0, transition: { duration: 0.6, ease: [0.22, 1, 0.36, 1], delay: d } }),
};
...
function FAQItem({ faq, index }) {
  ...
  return (
    <div className="border-b border-gray-100 last:border-0">
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-start justify-between gap-4 py-4.5 py-4 text-left group"
        aria-expanded={open}
      >
        <span className={`text-[13.5px] font-semibold leading-snug transition-colors duration-150 ${open ? "text-[#0818A8]" : "text-gray-800 group-hover:text-[#0818A8]"}`}>
          {faq.q}
        </span>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
            ...
            <p className="text-gray-700 text-[13.5px] font-light leading-relaxed pb-5 pr-8">{faq.a}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
// ═══════════════════════════════════════════════════════════════════════════════
// HERO — Search-first, clean, direct
// ═══════════════════════════════════════════════════════════════════════════════
function Hero({ onSearch, searchQuery, setSearchQuery }) {
  ...
  return (
    <section className="relative bg-[#00061a] overflow-hidden" aria-labelledby="support-hero-heading">
      {/* Grid */}
      ...
      <div ref={ref} className="relative z-10 max-w-[1380px] mx-auto px-5 sm:px-8 xl:px-10 pt-[100px] pb-16 md:pb-20">
        ...
        <motion.nav aria-label="Breadcrumb" className="flex items-center gap-2 mb-10"
          ...
          <span className="text-white/80 text-[11.5px] font-medium" aria-current="page">Support</span>
        </motion.nav>
        ...
        <div className="text-center max-w-2xl mx-auto">
          ...
          <motion.h1 id="support-hero-heading"
            className="font-black text-[clamp(34px,6vw,68px)] text-white leading-[0.88] tracking-[-0.035em] uppercase mb-5"
            initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.65, delay: 0.1 }}>
            How Can We<br />
          ...
          <motion.p className="text-white/65 text-[15px] font-light leading-relaxed mb-10"
            initial={{ opacity: 0, y: 12 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.55, delay: 0.22 }}>
            Search our help articles, track your shipment, or get directly in touch with our UK-based team.
          </motion.p>

          {/* ── SEARCH BAR ── */}
          <motion.div className="relative max-w-xl mx-auto mb-10"
            initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.55, delay: 0.35 }}>
            <div className="relative">
              <Search size={18} className="absolute left-5 top-1/2 -translate-y-1/2 text-white/40 pointer-events-none" aria-hidden="true" />
              <input
                type="search"
                ...
                onKeyDown={handleKey}
                className="w-full bg-white/[0.08] border border-white/[0.15] text-white text-[14px] font-light placeholder-white/30 pl-13 pr-[130px] pl-12 py-4 outline-none focus:border-[#1F51FF]/60 focus:bg-white/[0.12] focus:ring-1 focus:ring-[#1F51FF]/20 transition-all duration-200 rounded-none"
                aria-label="Search support articles"
              />
              <button
                onClick={onSearch}
                className="absolute right-2 top-1/2 -translate-y-1/2 bg-[#0818A8] hover:bg-[#0437F2] text-white text-[11.5px] font-black tracking-[0.08em] uppercase px-5 py-2 transition-all duration-200"
                aria-label="Search">
                Search
              </button>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-2 mt-4">
              {["Track shipment", "Customs duty", "Booking help", "Make a claim", "Delivery times"].map(t => (
                <button key={t} onClick={() => { setSearchQuery(t); onSearch(t); }}
                  className="text-[11px] font-medium text-white/45 hover:text-white border border-white/[0.1] hover:border-white/30 px-3 py-1.5 transition-all duration-150 bg-white/[0.03] hover:bg-white/[0.07]"
          ...
          {/* Quick action cards */}
          <motion.div className="grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-xl mx-auto"
              ...
              return (
                <a key={item.label} href={item.href}
                  ...
                  <div className="absolute top-0 left-0 right-0 h-[2px] w-0 group-hover:w-full transition-all duration-400"
                    style={{ backgroundColor: item.color }} aria-hidden="true" />
                  <div className="w-9 h-9 flex items-center justify-center flex-shrink-0"
                    style={{ backgroundColor: `${item.color}20` }} aria-hidden="true">
                    <Icon size={15} style={{ color: item.color }} />
                  </div>
                  <div className="text-left">
                    <p className="text-white/85 font-bold text-[13px] leading-tight">{item.label}</p>
                    <p className="text-white/45 text-[11px] font-light">{item.desc}</p>
                  </div>
                </a>
              );
            })}
...
function SystemStatus() {
  ...
  return (
    <div ref={ref} className={`border-b ${hasIssue ? "bg-amber-50 border-amber-200" : "bg-emerald-50 border-emerald-200"}`}
      role="region" aria-label="Service status">
      <div className="max-w-[1380px] mx-auto px-5 sm:px-8 xl:px-10 py-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <div className="flex items-center gap-2 flex-shrink-0">
            <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${hasIssue ? "bg-amber-500" : "bg-emerald-500"}`} aria-hidden="true" />
            <span className={`text-[11.5px] font-bold ${hasIssue ? "text-amber-800" : "text-emerald-800"}`}>
          ...
          <div className="flex flex-wrap gap-3">
            {STATUS_ITEMS.map((item, i) => (
              <motion.div key={item.label} className="flex items-center gap-1.5"
                initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ delay: i * 0.05 }}>
                <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${item.status === "operational" ? "bg-emerald-500" : item.status === "delayed" ? "bg-amber-500" : "bg-red-500"}`} aria-hidden="true" />
                <span className="text-[11px] font-medium text-gray-700">{item.label}</span>
              </motion.div>
            ))}
          </div>
          <a href="#" className="text-[11px] font-semibold text-[#0818A8] hover:text-[#0437F2] transition-colors flex-shrink-0 flex items-center gap-1 whitespace-nowrap"
...
function TrackWidget() {
  ...
  const statusColors = { "In Transit": "bg-amber-50 text-amber-700 border-amber-200", "Delivered": "bg-emerald-50 text-emerald-700 border-emerald-200" };

  return (
    <section id="track" ref={ref} className="relative bg-white scroll-mt-20 overflow-hidden" aria-labelledby="track-heading">
      ...
      <div className="relative z-10 max-w-[1380px] mx-auto px-5 sm:px-8 xl:px-10 py-16 md:py-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
            ...
            <h2 id="track-heading" className="font-black text-[clamp(24px,4vw,42px)] text-gray-900 leading-[0.92] tracking-[-0.025em] uppercase mb-4">
              Track Your<br />
            ...
            <p className="text-gray-600 text-[14px] font-light leading-relaxed mb-7">
              Enter your R-Zone booking reference to see exactly where your cargo is right now. Your reference looks like: <code className="text-gray-900 font-semibold bg-gray-100 px-2 py-0.5 text-[12px]">RZC-2024-00847</code>
            </p>

            {/* Track input */}
            <div className="flex gap-0 mb-4">
              <div className="relative flex-1">
                <Hash size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" aria-hidden="true" />
                <input
                  type="text"
                  ...
                  className="w-full border border-gray-300 border-r-0 text-gray-900 text-[14px] font-mono placeholder-gray-400 pl-10 pr-4 py-3.5 outline-none focus:border-[#0818A8] focus:ring-1 focus:ring-[#0818A8]/20 transition-all duration-200 bg-white"
                  aria-label="Enter booking reference"
                />
              </div>
              <button onClick={handleTrack} disabled={trackStatus === "loading"}
                className="flex items-center gap-2 bg-[#0818A8] hover:bg-[#0437F2] disabled:opacity-60 text-white text-[12px] font-black tracking-[0.08em] uppercase px-6 py-3.5 transition-all duration-200 flex-shrink-0"
            ...
            <div className="flex flex-wrap gap-2">
              <span className="text-[11px] text-gray-400 font-light flex-shrink-0 self-center">Try:</span>
              {Object.keys(MOCK_SHIPMENTS).map(ref => (
                <button key={ref} onClick={() => { setTrackInput(ref); }}
                  className="text-[11px] font-semibold text-[#0818A8] border border-[#0818A8]/20 bg-[#0818A8]/5 hover:bg-[#0818A8]/10 px-2.5 py-1 transition-colors"
          ...
          {/* Right — result */}
          <motion.div initial={{ opacity: 0, x: 16 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.65, delay: 0.2 }}>
            <AnimatePresence mode="wait">
              ...
              {trackStatus === "idle" && (
                <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  ...
                  <p className="text-gray-500 text-[13px] font-light">Your booking reference is in your confirmation email or text message from R-Zone.</p>
                </motion.div>
              )}
              ...
              {trackStatus === "notfound" && (
                <motion.div key="notfound" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
                  ...
                  <p className="text-red-700 text-[13px] font-light mb-5">We couldn't find a shipment with that reference. Please check the format and try again.</p>
                  <a href="tel:+448007720864"
                    className="inline-flex items-center gap-2 bg-[#0818A8] text-white text-[11.5px] font-black tracking-[0.08em] uppercase px-5 py-2.5 hover:bg-[#0437F2] transition-colors"
                ...
                <motion.div key="found" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-start justify-between mb-5">
                      <div>
                        <p className="text-[9.5px] font-bold tracking-[0.25em] uppercase text-gray-400 mb-1">Booking Reference</p>
                        <p className="font-mono font-bold text-[15px] text-gray-900">{trackInput.trim().toUpperCase()}</p>
                      </div>
                      <span className={`inline-flex items-center gap-1.5 px-3 py-1 border text-[10.5px] font-bold rounded-full ${statusColors[trackResult.status] || "bg-gray-100 text-gray-600 border-gray-200"}`}>
                    ...
                    <div className="flex items-center gap-3 border border-gray-100 bg-gray-50 px-4 py-3 mb-6">
                      <div className="text-center flex-shrink-0">
                        <p className="text-[10px] font-bold tracking-[0.15em] uppercase text-gray-400 mb-0.5">From</p>
                        <p className="font-bold text-[12.5px] text-gray-800">{trackResult.origin}</p>
                      </div>
                      ...
                      <div className="text-center flex-shrink-0">
                        <p className="text-[10px] font-bold tracking-[0.15em] uppercase text-gray-400 mb-0.5">To</p>
                        <p className="font-bold text-[12.5px] text-gray-800">{trackResult.destination}</p>
                      </div>
                    </div>
                    {/* Timeline */}
                      ...
                      {trackResult.timeline.map((step, i) => (
                        <div key={i} className="flex items-start gap-3.5 relative">
                          {i < trackResult.timeline.length - 1 && (
                            <div className={`absolute left-[11px] top-[24px] w-px h-full ${step.done ? "bg-[#0818A8]/30" : "bg-gray-100"}`} aria-hidden="true" />
                          )}
                          ...
                          <div className={`pb-4 flex-1 flex items-start justify-between ${step.done ? "opacity-100" : step.active ? "opacity-100" : "opacity-40"}`}>
                            <p className={`text-[13px] font-${step.done || step.active ? "semibold" : "light"} ${step.active ? "text-[#0818A8]" : "text-gray-800"}`}>
                              {step.label}
                            </p>
                            <div className="text-right flex-shrink-0 ml-4">
                              <p className={`text-[11.5px] font-${step.done ? "semibold" : "light"} text-gray-600`}>{step.date}</p>
                              {step.time !== "—" && <p className="text-[10.5px] font-light text-gray-400">{step.time}</p>}
                            </div>
                          </div>
                        </div>
                      ))}
                    ...
                    <div className="border-t border-gray-100 pt-4 mt-2 grid grid-cols-2 gap-3">
                      {[["Service", trackResult.service], ["Weight", trackResult.weight]].map(([k, v]) => (
                        <div key={k}>
                          <p className="text-[9.5px] font-bold tracking-[0.2em] uppercase text-gray-400">{k}</p>
...
function FAQSection({ searchResults, searchQuery, clearSearch }) {
  ...
  return (
    <section id="faqs" ref={ref} className="relative bg-[#00061a] scroll-mt-20 overflow-hidden" aria-labelledby="faq-heading">
      ...
      <div className="relative z-10 max-w-[1380px] mx-auto px-5 sm:px-8 xl:px-10 py-20 md:py-28">
        ...
        <div className="text-center mb-12">
          ...
          <motion.h2 id="faq-heading" className="font-black text-[clamp(26px,4.5vw,50px)] text-white leading-[0.92] tracking-[-0.028em] uppercase"
        ...
        <AnimatePresence>
          {searchQuery && searchResults.length > 0 && (
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mb-10">
              <div className="flex items-center justify-between mb-5">
                <p className="text-white/70 text-[13.5px] font-light">
                  <span className="text-white font-semibold">{searchResults.length} result{searchResults.length !== 1 ? "s" : ""}</span> for "{searchQuery}"
                </p>
                <button onClick={clearSearch} className="flex items-center gap-1.5 text-[11.5px] font-semibold text-white/50 hover:text-white transition-colors" aria-label="Clear search results">
          ...
          {searchQuery && searchResults.length === 0 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <HelpCircle size={32} className="text-white/25 mx-auto mb-3" aria-hidden="true" />
              <p className="text-white/70 font-semibold text-[15px] mb-1">No results for "{searchQuery}"</p>
              <p className="text-white/40 text-[13px] font-light mb-5">Try different keywords, or contact our team directly.</p>
              <button onClick={clearSearch} className="text-[12px] font-semibold text-[#1F51FF] hover:text-white transition-colors" aria-label="Clear search">Clear and browse all FAQs</button>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {!searchQuery && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-3" role="tablist" aria-label="FAQ categories">
              <p className="text-[9px] font-black tracking-[0.35em] uppercase text-white/28 mb-4 px-1">Categories</p>
              <div className="space-y-1">
                {FAQ_CATEGORIES.map(cat => {
                  const Icon = cat.icon;
                  const isActive = activeCategory === cat.id;
                  return (
                    <button key={cat.id} role="tab" aria-selected={isActive} aria-controls={`tab-${cat.id}`}
                      onClick={() => setActiveCategory(cat.id)}
                      className={`w-full flex items-center gap-3 px-4 py-3 text-left border-l-2 transition-all duration-150 ${
                        isActive ? "border-[#1F51FF] bg-white/[0.06] text-white" : "border-transparent text-white/55 hover:text-white/80 hover:bg-white/[0.04]"
                      }`}>
                      <Icon size={14} style={{ color: isActive ? cat.color : undefined }} className={!isActive ? "text-white/30" : ""} aria-hidden="true" />
                      <div>
                        <p className={`text-[12.5px] font-semibold ${isActive ? "text-white" : "text-white/65"}`}>{cat.label}</p>
                        <p className={`text-[10.5px] font-light ${isActive ? "text-white/55" : "text-white/30"}`}>{cat.faqs.length} questions</p>
                      </div>
                    </button>
                  );
                })}
            ...
            <div className="lg:col-span-9">
              <AnimatePresence mode="wait">
                <motion.div key={activeCategory} id={`tab-${activeCategory}`} role="tabpanel"
                  initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
                  transition={{ duration: 0.25 }}
                  className="border border-white/[0.09] bg-white/[0.03] overflow-hidden">
                  <div className="h-[2px]" style={{ background: `linear-gradient(to right, ${currentCategory?.color}, #1F51FF)` }} aria-hidden="true" />
                  <div className="divide-y divide-white/[0.06]">
                    {currentCategory?.faqs.map((faq, i) => (
                      <div key={i} className="px-6 [&_.text-gray-700]:text-white/70 [&_.text-gray-800]:text-white/90 [&_.text-gray-400]:text-white/30">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-1">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
              ...
              <h2 id="guides-heading" className="font-black text-[clamp(22px,3.5vw,38px)] text-gray-900 leading-[0.92] tracking-[-0.025em] uppercase mb-4">
              ...
              <div className="border border-gray-200 bg-gray-50 p-5">
                <p className="text-[9.5px] font-bold tracking-[0.25em] uppercase text-gray-500 mb-4">Popular Articles</p>
                <div className="space-y-3">
                  {POPULAR_ARTICLES.map((a, i) => (
                    <a key={i} href={a.href}
                      className="group flex items-start gap-2.5 text-[12.5px] text-gray-700 hover:text-[#0818A8] font-light transition-colors">
                      <ChevronRight size={12} className="flex-shrink-0 mt-0.5 text-gray-300 group-hover:text-[#0818A8] transition-colors" aria-hidden="true" />
                      <span>{a.title}</span>
                    </a>
                  ))}
                </div>
          ...
          <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {GUIDES.map((guide, i) => {
              ...
              return (
                <motion.a key={i} href={guide.href}
                  ...
                  initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.55, delay: i * 0.1 }}
                  ...
                  <p className="text-gray-600 text-[12.5px] font-light leading-relaxed mb-4">{guide.desc}</p>
                  <div className="flex items-center justify-between">
                    <span className="text-gray-400 text-[11px] font-light flex items-center gap-1.5">
                      <Clock size={10} aria-hidden="true" /> {guide.time}
                    </span>
                    <span className="flex items-center gap-1 text-[11px] font-bold text-[#0818A8] group-hover:text-[#0437F2] transition-colors">
...
function ContactSection() {
  ...
  return (
    <section id="contact" ref={ref} className="relative bg-[#00061a] scroll-mt-20 overflow-hidden" aria-labelledby="contact-support-heading">
      <div className="absolute inset-0 opacity-[0.022] pointer-events-none" aria-hidden="true"
        style={{ backgroundImage: "linear-gradient(rgba(255,255,255,1) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,1) 1px,transparent 1px)", backgroundSize: "64px 64px" }} />
      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#0818A8]/40 to-transparent pointer-events-none" aria-hidden="true" />
      ...
      <div className="relative z-10 max-w-[1380px] mx-auto px-5 sm:px-8 xl:px-10 py-20 md:py-28">
        ...
        <div className="text-center mb-14">
          ...
          <motion.h2 id="contact-support-heading" className="font-black text-[clamp(26px,4.5vw,50px)] text-white leading-[0.92] tracking-[-0.028em] uppercase"
            initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.1 }}>
            Talk to a Real{" "}
            <span className="relative inline-block text-[#1F51FF]">
              Person.
          ...
          <motion.p className="text-white/65 text-[14px] font-light mt-4 max-w-lg mx-auto leading-relaxed"
            initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.5, delay: 0.2 }}>
            Our UK-based team responds every time — no bots, no ticket queues. Call, WhatsApp, or email.
          </motion.p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
          {CONTACT_CHANNELS.map((ch, i) => {
            const Icon = ch.icon;
            return (
              <motion.a key={ch.title} href={ch.href}
                ...
                initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.55, delay: i * 0.08 }}
                aria-label={`${ch.title}: ${ch.value}`}>
                <div className="absolute top-0 left-0 right-0 h-[2px] w-0 group-hover:w-full transition-all duration-500"
                  style={{ backgroundColor: ch.color }} aria-hidden="true" />
                {ch.badge && (
                  <span className="absolute top-4 right-4 text-[9px] font-bold tracking-[0.18em] uppercase px-2 py-0.5 border"
                ...
                <div className="w-11 h-11 flex items-center justify-center mb-5"
                  style={{ backgroundColor: `${ch.color}18` }} aria-hidden="true">
                  <Icon size={18} style={{ color: ch.color }} />
                </div>
                <p className="text-[9.5px] font-bold tracking-[0.22em] uppercase text-white/32 mb-1">{ch.desc}</p>
                <p className="text-white font-black text-[15px] leading-tight mb-1.5">{ch.title}</p>
                <p className="text-white/60 text-[12.5px] font-semibold mb-1 break-all">{ch.value}</p>
                <p className="text-white/32 text-[11px] font-light mb-5 leading-snug">{ch.sub}</p>
                <span className="flex items-center gap-1.5 text-[10.5px] font-bold tracking-[0.08em] uppercase transition-colors group-hover:text-white"
        ...
        <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-5"
          initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.45 }}>
          {[
            {
              flag: "🇬🇧", country: "UK Office — Essex",
              addr: "Unit 10 Moorhen Yard, Elms Lane, Upminster, Essex RM14 3TS",
              hours: [{ d: "Mon–Fri", t: "9:00am – 6:00pm" }, { d: "Saturday", t: "10:00am – 2:00pm" }, { d: "Sunday", t: "Closed" }],
              color: "#0818A8",
            },
            {
              flag: "🇳🇬", country: "Nigeria Hub — Lagos",
              addr: "2 Esan Olusegun Close, Igando, Lagos State",
              hours: [{ d: "Mon–Fri", t: "8:00am – 5:00pm WAT" }, { d: "Saturday", t: "9:00am – 1:00pm WAT" }, { d: "Sunday", t: "Closed" }],
              color: "#1F51FF",
            },
          ].map(office => (
            <div key={office.country}
              ...
              <div className="h-[2px] w-0 group-hover:w-full transition-all duration-500 absolute top-0 left-0"
                style={{ backgroundColor: office.color }} aria-hidden="true" />
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-white/55 text-[12px] font-light mt-1 leading-snug">{office.addr}</p>
                </div>
              </div>
              <div className="border-t border-white/[0.07] pt-4 grid grid-cols-3 gap-2">
                {office.hours.map(h => (
                  <div key={h.d}>
                    <p className="text-white/32 text-[9.5px] font-bold tracking-[0.1em] uppercase">{h.d}</p>
                    <p className={`text-[11.5px] font-semibold mt-0.5 ${h.t === "Closed" ? "text-white/30" : "text-white/80"}`}>{h.t}</p>
                  </div>
                ))}
              </div>
            </div>
...
function EscalationCTA() {
  ...
  return (
    <section ref={ref} className="relative bg-white overflow-hidden" aria-label="Escalation and claims">
      <div className="absolute inset-0 pointer-events-none opacity-40" aria-hidden="true"
        style={{ backgroundImage: "radial-gradient(circle, rgba(8,24,168,0.04) 1px, transparent 1px)", backgroundSize: "44px 44px" }} />

      <div className="relative z-10 max-w-[1380px] mx-auto px-5 sm:px-8 xl:px-10 py-16 md:py-18">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">

          <motion.div className="border border-[#0818A8]/15 bg-[#0818A8]/4 p-7 relative overflow-hidden group hover:border-[#0818A8]/35 transition-all duration-300"
            initial={{ opacity: 0, y: 18 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.55 }}>
            ...
            <p className="text-gray-600 text-[13px] font-light leading-relaxed mb-5">Cargo arrived damaged? Report within 7 days for a full investigation and claim.</p>
            <a href="mailto:claims@r-zoneenterprises.com"
              className="inline-flex items-center gap-2 text-[11.5px] font-black tracking-[0.08em] uppercase text-[#0818A8] hover:text-[#0437F2] transition-colors"
              aria-label="Email claims team about damaged cargo">
              Report Damage <ArrowRight size={10} aria-hidden="true" />
            </a>
          </motion.div>

          <motion.div className="border border-amber-200 bg-amber-50 p-7 relative overflow-hidden group hover:border-amber-300 transition-all duration-300"
            initial={{ opacity: 0, y: 18 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.55, delay: 0.1 }}>
            ...
            <p className="text-gray-600 text-[13px] font-light leading-relaxed mb-5">Cargo not arrived? Let us investigate. Contact us if you're outside the expected delivery window.</p>
            <a href="tel:+448007720864"
              className="inline-flex items-center gap-2 text-[11.5px] font-black tracking-[0.08em] uppercase text-amber-700 hover:text-amber-900 transition-colors"
              aria-label="Call R-Zone about delayed cargo">
              Call Us Now <ArrowRight size={10} aria-hidden="true" />
            </a>
          </motion.div>

          <motion.div className="bg-[#0818A8] p-7 relative overflow-hidden"
            initial={{ opacity: 0, y: 18 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.55, delay: 0.2 }}>
            <div className="absolute inset-0 opacity-[0.07]" aria-hidden="true"
              style={{ backgroundImage: "linear-gradient(rgba(255,255,255,1) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,1) 1px,transparent 1px)", backgroundSize: "32px 32px" }} />
            <div className="relative z-10">
              <LifeBuoy size={22} className="text-white/70 mb-4" aria-hidden="true" />
              <h3 className="text-white font-black text-[16px] tracking-[-0.01em] mb-2">Make a Formal Complaint</h3>
              <p className="text-white/70 text-[13px] font-light leading-relaxed mb-5">Not satisfied? Our complaints team aims to resolve all cases within 14 working days.</p>
              <a href="mailto:complaints@r-zoneenterprises.com"
                className="inline-flex items-center gap-2 bg-white text-[#0818A8] text-[11.5px] font-black tracking-[0.08em] uppercase px-5 py-2.5 hover:bg-white/90 transition-colors"
    ...
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "@graph": [
          ...
          { "@type": "FAQPage", "mainEntity": FAQ_CATEGORIES.flatMap(cat =>
            cat.faqs.map(faq => ({
              "@type": "Question",
              "name": faq.q,
